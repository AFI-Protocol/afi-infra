...
describe("governed-schema validation v2 (vendored FACTORY-CONTRACT)", () => {
  ...
  it("carries the governed base contract + REQUIRED composition ($ref afi.composition-ref.v1)", () => {
    ...
    // the governed base evidence properties are all present
    [
      "schema",
      "signalId",
      "analystId",
  ...
  it("REJECTS a record carrying the v1 schema const (v2 is the only contract)", () => {
    ...
    expect(validateEvidenceSchemaV2(v1Shaped).valid).toBe(false);
    // Even WITH composition, the v1 const alone is inadmissible.
    const v1Const: any = validBaseV2();
    v1Const.schema = "afi.scored-signal-evidence.v1";
    expect(validateEvidenceSchemaV2(v1Const).valid).toBe(false);
  });
});
...
describe("scoring-profile stamp (PR-UWR-STAMP / RC-6)", () => {
  it("local UwrProfileStampSource union matches the governed source enum", () => {
    ...
    expect(vendoredEvidenceSchemaV2.properties.uwrProfile.properties.source.enum).toEqual(
      LOCAL_STAMP_SOURCES
    );
  });

  it("REQUIRES the stamp on every canonical evidence record", () => {
    expect(vendoredEvidenceSchemaV2.required).toContain("uwrProfile");
    const { uwrProfile: _omitted, ...unstamped } = validBaseV2() as any;
    const { valid, errors } = validateEvidenceSchemaV2(unstamped);
    expect(valid).toBe(false);
  ...
  it("accepts BOTH governed sources and rejects an ungoverned one", () => {
    for (const source of LOCAL_STAMP_SOURCES) {
      ...
      expect(validateEvidenceSchemaV2(rec).valid, source).toBe(true);
    }
    for (const bad of ["registry", "builtin", "unknown", ""]) {
      const rec: any = validBaseV2();
      rec.uwrProfile.source = bad;
      expect(validateEvidenceSchemaV2(rec).valid, `source '${bad}'`).toBe(false);
    }
  });

  it("rejects a stamp with missing source or malformed profile metadata", () => {
    const noSource: any = validBaseV2();
    delete noSource.uwrProfile.source;
    expect(validateEvidenceSchemaV2(noSource).valid).toBe(false);
    for (const field of ["profileId", "status", "decisionRef"]) {
      const rec: any = validBaseV2();
      rec.uwrProfile[field] = "";
      expect(validateEvidenceSchemaV2(rec).valid, `empty ${field}`).toBe(false);
    }
  });
...
// afi-config source paths for each vendored schema file (the v2 evidence
// contract, composition-ref, and the shared provenance deps).
